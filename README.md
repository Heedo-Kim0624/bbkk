...
목록은 사이트를 사용하는 모두에게 공유됩니다. `공유됨`은 저장 완료 상태이며, 연결·저장 중에는 새 뽑기가 잠시 비활성화됩니다. 충돌 시 `최신 목록`과 `내 수정 적용` 중 선택하고, 연결 실패 시 `다시 연결`로 복구합니다. 이미 뽑은 알과 개인 기록은 다른 사용자의 편집에 영향을 받지 않습니다.
...
4. 배포 후 두 브라우저의 목록 동기화, 알 뽑기·공개, 새로고침 복원과 모바일 화면을 확인합니다. 동기화 확인용으로 만든 항목만 제거하여 기존 사용자 편집을 보존합니다.
...
벌칙 항목은 서버에 공유되지만 사용자 설정·당첨 제외·기록·아직 열지 않은 알은 현재 브라우저에만 보관됩니다. 브라우저 데이터를 지우면 개인 데이터는 사라집니다. 공유 목록이 오프라인이어도 이미 뽑은 알은 열 수 있습니다. 이전 기기 전용 목록은 자동 업로드하지 않고 보존하며, 도움말의 `이 기기의 목록 가져오기`로 공유 목록에 추가할 수 있습니다. 목록 초기화는 모든 사용자에게 반영됩니다.
