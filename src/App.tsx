...
function App() {
  ...
  const draw = useCallback(() => {
    if (drawingRef.current || state.pending || modal) return
  ...
  function addItem() {
    if (drawingRef.current || locked || state.items.length >= MAX_ITEMS) return
    setNotice(null)
  ...
  function resetItems(useDefaults: boolean) {
    ...
    setState(previous => ({ ...previous, items: useDefaults ? DEFAULT_ITEMS.map(item => ({ ...item })) : [], history: [], excludedIds: [], pending: null }))
    setActiveTier('high')
    setResult(null)
    setModal(null)
  ...
  return (
    <div className="app-shell">
        ...
        <div className="studio-grid" id="draw-studio">
          <section className={`draw-panel glass-panel ${spinning ? 'is-spinning' : ''} ${result ? 'has-result' : ''} ${state.pending && !spinning ? 'has-egg' : ''}`} aria-labelledby="draw-title">
            ...
            <div className="draw-bottom">
              <div className="draw-action"><Button label={spinning ? '뽑는 중…' : state.pending ? '알을 열어주세요' : result ? '다시 뽑기' : '알 뽑기'} variant="primary" size="lg" width="100%" isDisabled={locked || missingTiers.length > 0} icon={<Shuffle size={20} />} onClick={draw} /><ArrowRight className="draw-arrow" size={18} /></div>
              <p className="draw-hint">{locked ? '\u00a0' : missingTiers.length ? `${missingLabels} 등급에 항목이 필요해요.` : <span className="desktop-shortcut"><kbd title="Space 키로 뽑기">Space</kbd></span>}</p>
            </div>
          ...
          <section className="editor-panel glass-panel" ref={editorRef} aria-labelledby="editor-title">
            <div className="editor-heading"><Stack direction="horizontal" gap={2} vAlign="center"><SlidersHorizontal size={19} /><h2 id="editor-title">벌칙</h2><span className="count-badge">{state.items.length}</span></Stack><button className="text-button" aria-label="초기화" disabled={locked} onClick={() => setModal('new')}><RotateCcw size={13} /> 초기화</button></div>
            ...
            {TIERS.map(group => <div key={group.id} id={`panel-${group.id}`} role="tabpanel" aria-label={`${group.label} 벌칙`} hidden={activeTier !== group.id}>{activeTier === group.id && <div className="item-list">{visibleItems.length === 0 ? <div className="empty-items"><ListPlus size={30} strokeWidth={1.2} /><p>벌칙을 추가하세요.</p></div> : visibleItems.map((item, index) => {
              ...
              return <article className={`item-row ${isExcluded || disabledItem ? 'is-excluded' : ''}`} key={item.id} data-item-id={item.id} style={{ '--item-color': tier.color } as CSSProperties}>
                <div className="item-main"><span className="item-number">{isExcluded ? <Check size={12} /> : String(index + 1).padStart(2, '0')}</span><TextInput label={`항목 ${index + 1} 이름`} isLabelHidden value={item.label} onChange={label => updateItem(item.id, { label: label.slice(0, MAX_LABEL_LENGTH) })} placeholder="벌칙 입력" size="sm" isDisabled={locked} width="100%" /><span className="item-percentage" aria-label={missingTiers.length ? '등급별 항목을 채우면 추첨 가능' : `실제 당첨 확률 ${percentage(odds[item.id] ?? 0)}`}>{isExcluded || disabledItem ? '제외' : missingTiers.length ? '—' : percentage(odds[item.id] ?? 0)}</span><button className="remove-item icon-button" aria-label={`항목 ${index + 1} 삭제`} title="항목 삭제" disabled={locked} onClick={() => removeItem(item.id)}><X size={15} /></button></div>
                <div className="item-options"><label className="item-enabled"><input type="checkbox" checked={!disabledItem} disabled={locked} aria-label={`항목 ${index + 1} 추첨 포함`} onChange={event => updateItem(item.id, { enabled: event.target.checked })} />포함</label><Selector label={`항목 ${index + 1} 등급`} isLabelHidden options={TIERS.map(tier => ({ value: tier.id, label: tier.label }))} value={item.tier} onChange={value => { const target = TIERS.find(tier => tier.id === value); if (target) { updateItem(item.id, { tier: target.id, color: target.color }); setNotice({ message: `${target.label}(으)로 이동됨` }) } }} isDisabled={locked} size="sm" variant="ghost" width={80} /></div>
              </article>
            })}</div>}</div>)}

            <div className="add-actions"><button className="add-item" aria-label="항목 추가" disabled={locked || state.items.length >= MAX_ITEMS} onClick={addItem}><Plus size={17} /> 추가</button><button className="bulk-add" disabled={locked || state.items.length >= MAX_ITEMS} onClick={() => { setBulkError(''); setModal('bulk') }} aria-label="여러 항목 한 번에 추가" title="여러 항목 한 번에 추가"><ListPlus size={19} /></button></div>
      ...
      {notice && <div className="toast" role="status"><Check size={17} /><span>{notice.message}</span>{notice.undo && <button disabled={locked} onClick={() => { notice.undo?.(); setNotice(null) }}>되돌리기</button>}<button className="toast-close" aria-label="알림 닫기" onClick={() => setNotice(null)}><X size={14} /></button></div>}
      ...
      <dialog className="app-dialog" ref={dialogRef} onCancel={() => setModal(null)} onClose={() => setModal(null)} onClick={event => { if (event.target === event.currentTarget) { const rect = event.currentTarget.getBoundingClientRect(); if (event.clientX < rect.left || event.clientX > rect.right || event.clientY < rect.top || event.clientY > rect.bottom) setModal(null) } }} aria-labelledby="dialog-title">
        ...
        {modal === 'help' && <><h2 id="dialog-title">이용 방법</h2><ul className="help-summary"><li>상 60% · 중 35% · 하 4.9% · 극하 0.1%</li><li>알을 뽑고, 눌러서 벌칙을 확인하세요.</li><li>같은 등급의 벌칙은 동일 확률로 나와요.</li><li>네 등급마다 추첨 가능한 항목이 필요해요.</li><li>‘중복 제외’로 비워진 등급은 ‘전체 포함’으로 복원하세요.</li></ul><p className="privacy-note">최대 30개 · 항목, 알, 기록은 이 브라우저에 저장됩니다.</p><Button label="닫기" variant="primary" width="100%" onClick={() => setModal(null)} /></>}
