...
test('responsive studio renders without overflow or browser errors', async ({ page }) => {
  ...
  for (const width of [320, 390, 768, 1440]) {
    ...
    await expect(page.getByRole('button', { name: '벌칙 뽑기', exact: true })).toBeVisible()
...
test('text editing, weight normalization, slider, deletion and persistence', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: '벌칙 뽑기', exact: true })).toBeDisabled()
  ...
  await page.getByRole('button', { name: '동일하게' }).click()
...
test('draw is deterministic with one eligible item; excludes, exhausts, resets and restores history', async ({ page }) => {
  ...
  await page.getByRole('switch', { name: '중복 제외' }).click()
  await page.getByRole('button', { name: '벌칙 뽑기', exact: true }).click()
  ...
  await expect(page.getByRole('button', { name: '다시 시작', exact: true })).toBeEnabled()
...
test('custom choices and history survive reload; running preset reset is explicit and reversible', async ({ page }) => {
  await page.emulateMedia({ reducedMotion: 'reduce' })
  await openStudio(page)
  ...
  await expect(page.locator('.item-percentage')).toHaveText(['20%', '20%', '20%', '20%', '20%'])
  await startEmpty(page)
  await addItems(page, ['내가 정한 벌칙'])
  await page.getByRole('button', { name: '벌칙 뽑기', exact: true }).click()
  await expect(page.locator('.history-list li')).toHaveCount(1)
  await page.reload()
