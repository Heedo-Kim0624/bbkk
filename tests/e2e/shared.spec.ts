...
test('preassigned card contents and history remain private while another browser edits the shared list', async ({ room }) => {
  const { a, b, store } = room
  await a.evaluate(() => {
  ...
  await a.getByRole('button', { name: '카드 뽑기', exact: true }).click()
  await expect(a.getByRole('button', { name: '3번 카드 선택', exact: true })).toBeEnabled()
  const pending = await a.evaluate(() => JSON.parse(localStorage.getItem('bbob-studio-v1')!).pending)
  expect(pending.kind).toBe('cards')
  expect(pending.cards).toHaveLength(5)
  await expect(b.locator('.tarot-choice:enabled')).toHaveCount(0)
  await expect(b.locator('.history-list li')).toHaveCount(0)
  await field(b, 1, '카드를 뽑은 뒤 다른 사람이 바꿈')
  await synced(b)
  await expect.poll(() => store.board.items[0].label).toBe('카드를 뽑은 뒤 다른 사람이 바꿈')
  await a.evaluate(() => window.dispatchEvent(new Event('focus')))
  await expectField(a, 1, '카드를 뽑은 뒤 다른 사람이 바꿈')
  expect(await a.evaluate(() => JSON.parse(localStorage.getItem('bbob-studio-v1')!).pending)).toEqual(pending)
  store.offline.add(a)
  await a.evaluate(() => window.dispatchEvent(new Event('online')))
  await expect(a.locator('.save-status')).toContainText('연결 끊김', { timeout: 10_000 })
  await a.getByRole('button', { name: '3번 카드 선택', exact: true }).click()
  await expect(a.locator('.result-card h3')).toHaveText('청소 20분')
  await expect(a.locator('.history-list li')).toHaveCount(1)
  await expect(a.locator('.other-card h4')).toHaveText(Array(4).fill('청소 20분'))
  await expect(b.locator('.history-list li')).toHaveCount(0)
